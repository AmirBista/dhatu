(function(window){'use strict';var keys={ENTER:13,ESC:27,F1:112,F12:123,LEFT:37,RIGHT:39};var defaults={modal:!0,basic:!1,frameless:!1,movable:!0,resizable:!0,closable:!0,closableByDimmer:!0,maximizable:!0,startMaximized:!1,pinnable:!0,pinned:!0,padding:!0,overflow:!0,maintainFocus:!0,transition:'pulse',autoReset:!0,notifier:{delay:5,position:'bottom-right'},glossary:{title:'Orbit',ok:'OK',cancel:'Cancel',acccpt:'Accept',deny:'Deny',confirm:'Confirm',decline:'Decline',close:'Close',maximize:'Maximize',restore:'Restore',},theme:{input:'ajs-input',ok:'ajs-ok',cancel:'ajs-cancel',}};var openDialogs=[];function addClass(element,classNames){element.className+=' '+classNames}
function removeClass(element,classNames){var classes=classNames.split(' ');for(var x=0;x<classes.length;x+=1){element.className=element.className.replace(' '+classes[x],'')}}
...
var buttonsDefinition=[];if(Array.isArray(setup.buttons)){for(var b=0;b<setup.buttons.length;b+=1){var ref=setup.buttons[b],copy={};for(var i in ref){if(ref.hasOwnProperty(i)){copy[i]=ref[i]}}
...
function maximize(instance){addClass(instance.elements.root,classes.maximized);if(instance.isOpen()){ensureNoOverflow()}}
function restore(instance){removeClass(instance.elements.root,classes.maximized);if(instance.isOpen()){ensureNoOverflow()}}
...
function handleTransitionInEvent(event,instance){clearTimeout(instance.__internal.timerIn);setFocus(instance);cancelKeyup=!1;if(typeof instance.get('onfocus')==='function'){instance.get('onfocus')()}
off(instance.elements.dialog,transition.type,instance.__internal.transitionInHandler);removeClass(instance.elements.root,classes.animationIn)}
...
var movable=null,offsetX=0,offsetY=0,xProp='pageX',yProp='pageY';function moveElement(event,element){element.style.left=(event[xProp]-offsetX)+'px';element.style.top=(event[yProp]-offsetY)+'px'}
function beginMove(event,instance){if(resizable===null&&!instance.isMaximized()&&instance.get('movable')){var eventSrc;if(event.type==='touchstart'){event.preventDefault();eventSrc=event.targetTouches[0];xProp='clientX';yProp='clientY'}else if(event.button===0){eventSrc=event}
if(eventSrc){movable=instance;offsetX=eventSrc[xProp];offsetY=eventSrc[yProp];var element=instance.elements.dialog;addClass(element,classes.capture);if(element.style.left){offsetX-=parseInt(element.style.left,10)}
if(element.style.top){offsetY-=parseInt(element.style.top,10)}
moveElement(eventSrc,element);addClass(document.body,classes.noSelection);return!1}}}
...
if(eventSrc){moveElement(eventSrc,movable.elements.dialog)}}}
function endMove(){if(movable){var element=movable.elements.dialog;movable=null;removeClass(document.body,classes.noSelection);removeClass(element,classes.capture)}}
...
if(eventSrc){resizable=instance;handleOffset=instance.elements.resizeHandle.offsetHeight/2;var element=instance.elements.dialog;addClass(element,classes.capture);startingLeft=parseInt(element.style.left,10);element.style.height=element.offsetHeight+'px';element.style.minHeight=instance.elements.header.offsetHeight+instance.elements.footer.offsetHeight+'px';element.style.width=(startingWidth=element.offsetWidth)+'px';if(element.style.maxWidth!=='none'){element.style.minWidth=(minWidth=element.offsetWidth)+'px'}
...
function endResize(){if(resizable){var element=resizable.elements.dialog;resizable=null;removeClass(document.body,classes.noSelection);removeClass(element,classes.capture);cancelClick=!0}}
...
return this},moveTo:function(x,y){if(!isNaN(x)&&!isNaN(y)){var element=this.elements.dialog,current=element,offsetLeft=0,offsetTop=0;if(element.style.left){offsetLeft-=parseInt(element.style.left,10)}
...
return this},resizeTo:function(width,height){if(!isNaN(width)&&!isNaN(height)&&this.get('resizable')===!0){var element=this.elements.dialog;if(element.style.maxWidth!=='none'){element.style.minWidth=(minWidth=element.offsetWidth)+'px'}
element.style.maxWidth='none';element.style.minHeight=this.elements.header.offsetHeight+this.elements.footer.offsetHeight+'px';element.style.width=width+'px';element.style.height=height+'px'}
...
return this}},set:function(key,value){this.setting(key,value);return this},get:function(key){return this.setting(key)},setHeader:function(content){if(typeof content==='string'){this.elements.header.innerHTML=content}else if(content instanceof window.HTMLElement&&this.elements.header.firstChild!==content){this.elements.header.innerHTML='';this.elements.header.appendChild(content)}
return this},setContent:function(content){if(typeof content==='string'){this.elements.content.innerHTML=content}else if(content instanceof window.HTMLElement&&this.elements.content.firstChild!==content){this.elements.content.innerHTML='';this.elements.content.appendChild(content)}
...
ensureNoOverflow();if(typeof className==='string'&&className!==''){this.__internal.className=className;addClass(this.elements.root,className)}
...
if(typeof this.get('onshow')==='function'){this.get('onshow')()}}else{resetMove(this);resetResize(this);addClass(this.elements.dialog,classes.shake);var self=this;setTimeout(function(){removeClass(self.elements.dialog,classes.shake)},200)}
...
if(typeof this.get('onclose')==='function'){this.get('onclose')()}
openDialogs.splice(openDialogs.indexOf(this),1);this.__internal.isOpen=!1;ensureNoOverflow()}
...
function create(div,callback){function clickDelegate(event,instance){instance.dismiss(!0)}
...
return this},setContent:function(content){if(typeof content==='string'){this.element.innerHTML=content}else{this.element.appendChild(content)}
...
break;case 'defaultFocus':this.__internal.focus.element=newValue==='ok'?0:1;break}},callback:function(closeEvent){clearAutoConfirm(this);var returnValue;switch(closeEvent.index){case 0:if(typeof this.get('onok')==='function'){returnValue=this.get('onok').call(undefined,closeEvent);if(typeof returnValue!=='undefined'){closeEvent.cancel=!returnValue}}
break;case 1:if(typeof this.get('oncancel')==='function'){returnValue=this.get('oncancel').call(undefined,closeEvent);if(typeof returnValue!=='undefined'){closeEvent.cancel=!returnValue}}
...
break;case 'reverseButtons':if(newValue===!0){this.elements.buttons.primary.appendChild(this.__internal.buttons[0].element)}else{this.elements.buttons.primary.appendChild(this.__internal.buttons[1].element)}
break}},callback:function(closeEvent){var returnValue;switch(closeEvent.index){case 0:this.value=input.value;if(typeof this.get('onok')==='function'){returnValue=this.get('onok').call(undefined,closeEvent,this.value);if(typeof returnValue!=='undefined'){closeEvent.cancel=!returnValue}}
break;case 1:if(typeof this.get('oncancel')==='function'){returnValue=this.get('oncancel').call(undefined,closeEvent);if(typeof returnValue!=='undefined'){closeEvent.cancel=!returnValue}}
